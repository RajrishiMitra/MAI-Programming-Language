...
class ExpectedCharError(Error):
    def __init__(self, pos_start, pos_end, details):
        super().__init__(pos_start, pos_end, 'Baka... Expected character \nNote: ', details)
...
class Lexer:
    ...
    def make_tokens(self):
        ...
        while self.curr_char != None:
            if self.curr_char in ' \t':
                ...
                tokens.append(Token(MAI_MUL, pos_start=self.pos))
                self.advance()
            elif self.curr_char == '/':
                tokens.append(Token(MAI_DIV, pos_start=self.pos))
                self.advance()
            elif self.curr_char == '^':
                tokens.append(Token(MAI_POW, pos_start=self.pos))
                self.advance()
            elif self.curr_char == '(':
                tokens.append(Token(MAI_LPAREN, pos_start=self.pos))
                self.advance()
            elif self.curr_char == ')':
                tokens.append(Token(MAI_RPAREN, pos_start=self.pos))
                self.advance()
            elif self.curr_char == '[':
                tokens.append(Token(MAI_LSQUARE, pos_start=self.pos))
                self.advance()
            elif self.curr_char == ']':
                tokens.append(Token(MAI_RSQUARE, pos_start=self.pos))
                self.advance()
            elif self.curr_char == '!':
                ...
                tokens.append(Token(MAI_COMMA, pos_start=self.pos))
                self.advance()
            else:
        ...
        tokens.append(Token(MAI_EOF, pos_start=self.pos))
        return tokens, None
    ...
    def make_string(self):
        ...
        self.advance()
        return Token(MAI_STRING, string, pos_start, self.pos)
    ...
    def make_not_equals(self):
        ...
        if self.curr_char == '=':
            self.advance()
            return Token(MAI_NE, pos_start=pos_start, pos_end=self.pos), None
...
class Parser:
    ...
    def parse(self):
        res = self.expr()
        if not res.error and self.current_tok.type != MAI_EOF:
    ...
    def expr(self):
        ...
        node = res.register(self.bin_op(
            self.comp_expr, ((MAI_KEYWORD, 'AND'), (MAI_KEYWORD, 'OR'))))

        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'MAI', 'IF', 'FOR', 'WHILE', 'FUN', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
            ))
    ...
    def comp_expr(self):
        ...
        node = res.register(self.bin_op(
            self.arith_expr, (MAI_EE, MAI_NE, MAI_LT, MAI_GT, MAI_LTE, MAI_GTE)))
    ...
    def power(self):
        return self.bin_op(self.call, (MAI_POW, ), self.factor)
    ...
    def atom(self):
        ...
        elif tok.type == MAI_LSQUARE:
            list_expr = res.register(self.list_expr())
            if res.error:
                return res
            return res.success(list_expr)

        elif tok.matches(MAI_KEYWORD, 'IF'):
            if_expr = res.register(self.if_expr())
            if res.error:
                return res
            return res.success(if_expr)

        elif tok.matches(MAI_KEYWORD, 'FOR'):
            for_expr = res.register(self.for_expr())
            if res.error:
                return res
            return res.success(for_expr)

        elif tok.matches(MAI_KEYWORD, 'WHILE'):
            while_expr = res.register(self.while_expr())
            if res.error:
                return res
            return res.success(while_expr)

        elif tok.matches(MAI_KEYWORD, 'FUN'):
    ...
    def list_expr(self):
        ...
        if self.current_tok.type != MAI_LSQUARE:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected '['"
            ))
    ...
    def if_expr(self):
        ...
        if not self.current_tok.matches(MAI_KEYWORD, 'IF'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'IF'"
            ))
        ...
        if not self.current_tok.matches(MAI_KEYWORD, 'THEN'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'THEN'"
            ))
    ...
    def for_expr(self):
        ...
        if not self.current_tok.matches(MAI_KEYWORD, 'FOR'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'FOR'"
            ))
        ...
        if not self.current_tok.matches(MAI_KEYWORD, 'TO'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'TO'"
            ))
    ...
    def while_expr(self):
        ...
        if not self.current_tok.matches(MAI_KEYWORD, 'WHILE'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'WHILE'"
            ))
    ...
    def func_def(self):
        ...
        if not self.current_tok.matches(MAI_KEYWORD, 'FUN'):
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                f"Expected 'FUN'"
            ))
...
class Interpreter:
    ...
    def visit_BinOpNode(self, node, context):
        ...
        elif node.op_tok.type == MAI_LT:
            result, error = left.get_comparison_lt(right)
        elif node.op_tok.type == MAI_GT:
            result, error = left.get_comparison_gt(right)
        elif node.op_tok.type == MAI_LTE:
            result, error = left.get_comparison_lte(right)
        elif node.op_tok.type == MAI_GTE:
            result, error = left.get_comparison_gte(right)
        elif node.op_tok.matches(MAI_KEYWORD, 'AND'):
            result, error = left.anded_by(right)
        elif node.op_tok.matches(MAI_KEYWORD, 'OR'):
