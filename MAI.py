    ...
    def make_tokens(self):
        ...
        while self.curr_char != None:
            if self.curr_char in ' \t':
                ...
                token.append(Token(MAI_LPAREN, pos_start=self.pos))
                self.advance()
            elif self.curr_char == ')':
                token.append(Token(MAI_RPAREN, pos_start=self.pos))
                self.advance()
            elif self.curr_char == '!':
                token1, error = self.make_not_equals()
                if error:
                    return [], error
                token.append(token1)
            elif self.curr_char == '=':
                token.append(self.make_equals())
            elif self.curr_char == '<':
                token.append(self.make_less_than())
            elif self.curr_char == '>':
                token.append(self.make_greater_than())
            else:
...
class Parser:
    ...
    def advance(self, ):
        ...
        if self.tok_idx < len(self.token):
            self.curr_tok = self.token[self.tok_idx]
    ...
    def if_expr(self):
        ...
        if not self.curr_tok.matches(MAI_KEYWORD, 'IF'):
            return res.failure(InvalidSyntaxError(
                self.curr_tok.pos_start, self.curr_tok.pos_end,
                f"Expected 'IF'"
            ))
        ...
        if not self.curr_tok.matches(MAI_KEYWORD, 'THEN'):
            return res.failure(InvalidSyntaxError(
                self.curr_tok.pos_start, self.curr_tok.pos_end,
                f"Expected 'THEN'"
            ))
        ...
        while self.curr_tok.matches(MAI_KEYWORD, 'ELIF'):
            res.register_advance()
            self.advance()
        ...
        if self.curr_tok.matches(MAI_KEYWORD, 'ELSE'):
            res.register_advance()
            self.advance()
    ...
    def atom(self):
        ...
        if tok.type in (MAI_INT, MAI_FLOAT):
            ...
            if res.error:
                return res
            if self.curr_tok.type == MAI_RPAREN:
                res.register_advance()
                self.advance()
    ...
    def comp_expr(self):
        ...
        if self.curr_tok.matches(MAI_KEYWORD, 'NOT'):
    ...
    def expr(self):
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.curr_tok.pos_start, self.curr_tok.pos_end,
                "Expected 'MAI', int, float, identifier, '+', '-', '(' or 'NOT'"
            ))
